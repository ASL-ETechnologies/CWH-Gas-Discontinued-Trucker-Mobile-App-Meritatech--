...
import static com.meritatech.myrewardzpos.database.ReflectionUtil.getDomainClasses;
...
//                }
    ...
    public void createTables(ArrayList<String> allClassNames) {
            ...
            try {
                if (className.startsWith("com.meritatech.myrewardzpos.data")) {
                    ...
                    if (DatabaseObject.class.isAssignableFrom(dbClass)
                            && dbClass.isAnnotationPresent(PersistAnnotation.class)) {
                        docreateTable(dbClass, sqLiteDatabase);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    protected void docreateTable(Class<?> table, SQLiteDatabase sqLiteDatabase) {

        try {
            boolean tableExists = tableExists(table, sqLiteDatabase);
            ...
            if (tableExists == false) {
                ...
                if (!createSQL.isEmpty()) {
                    try {
                        Log.i("PosApi-DB", "Creating table " + createSQL);
                        sqLiteDatabase.execSQL(createSQL);
                        generateIndexes(table, sqLiteDatabase);
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                }
    ...
    public static boolean tableExists(Class<?> table, SQLiteDatabase sqLiteDatabase) {
        String tableName = NamingHelper.toSQLName(table);

        String existsSQL = String.format("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='%s'", tableName);

        long result = -1;
        SQLiteStatement sqliteStatement;
        try {
            sqliteStatement = sqLiteDatabase.compileStatement(existsSQL);
    ...
    protected void generateIndexes(Class<?> table, SQLiteDatabase sqLiteDatabase) {
        List<Field> fields = ReflectionUtil.getTableFields(table);
        String tableName = NamingHelper.toSQLName(table);

        for (Field column : fields) {

            if (column.isAnnotationPresent(ColumnIndex.class)) {

                String columnName = NamingHelper.toSQLName(column);

                String createSQL = String.format("CREATE INDEX %s_i_%s ON %s (%s);", columnName, tableName, tableName, columnName);
                sqLiteDatabase.execSQL(createSQL);
            }
        }
    }
